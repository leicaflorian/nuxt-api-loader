import { SchemaValidationError }     from '../classes/errors/SchemaValidationError'
...
async function _getSchemaClass (schema) {
  ...
  const schemaPath = path.resolve(`${moduleSettings.schemasFolder}/${_capitalize(schema)}Schema.js`)
  ...
  const schemaClass = (await import(schemaPath)).default
...
async function _getValidationSchema (requestDetails) {
  ...
  try {
    ...
    if (!schemaInstance || !schemaInstance._requestedSchema) {
      return null
    }
  ...
  if (reqValidationSchema) {
    ...
    if (validationResult) {
      if (validationResult instanceof Error || validationResult.error) {
        const schemaError = new SchemaValidationError(validationResult.error || validationResult)
