...
class EmailRenderer {
  ...
  async render(tmplName, lang, data) {
    try {
      ...
      const tranlations = this._getLocalizedContent(tmplName, lang).content || {}
      ...
      if (mjmlResult.errors.length > 0) {
        let errorMessage = mjmlResult.errors

        if (mjmlResult.errors instanceof Array) {
          errorMessage = mjmlResult.errors.map(mess => mess.message || message)
        }
