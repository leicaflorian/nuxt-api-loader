...
module.exports = class ApiMiddleware {
  ...
  async _registerRoutes() {
    // Import API Routes
    const availableRoutes = fs.readdirSync(this._moduleOptions.routesFolder)

    for (const file of availableRoutes) {
      const fileExtension = path.extname(file)

      // tries to load only tJavascript files
      if (!fileExtension.match(/\.(m|)js/)){
        continue
      }

      const Module = (await import(path.resolve(this._moduleOptions.routesFolder, file))).default
